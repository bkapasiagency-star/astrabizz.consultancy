...
import { Layers, Factory, Gem, HardHat, Pill, ArrowUpRight, ChevronLeft, ChevronRight } from 'lucide-react';
            ...
            {CASE_STUDIES.map((cs) => {
              ...
              return (
                <SwiperSlide key={cs.id} className="h-auto">
                  <div className="card-lift h-full p-7 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 shadow-sm flex flex-col text-left hover:border-brand-primary/30">
                    ...
                    <span className="font-display text-base font-bold text-slate-900 dark:text-white mb-2.5 block leading-snug">
                      {cs.title}
                    </span>
                    <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-sans flex-1">
                      {cs.result}
                    </p>
                    <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-1.5 text-xs font-semibold text-brand-primary">
                      Verified engagement <ArrowUpRight className="w-3.5 h-3.5" />
                    </div>
                  </div>
                </SwiperSlide>
              );
