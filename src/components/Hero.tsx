...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // IMPORTANT: SplitType is only ever run on plain-text spans. Running it on
    // an element containing the gradient (bg-clip-text) span would move that
    // span's glyphs into new child nodes that inherit `color: transparent`
    // but NOT the parent's `background-image`/`background-clip`, making the
    // gradient headline invisible. So the plain run and the gradient run are
    // split independently and animated together as one stagger.
    const splitPlain = new SplitType(headlinePlainRef.current, { types: 'words', tagName: 'span' });
    const splitGradient = new SplitType(headlineGradientRef.current, { types: 'words', tagName: 'span' });
    const allWords = [...(splitPlain.words || []), ...(splitGradient.words || [])];
