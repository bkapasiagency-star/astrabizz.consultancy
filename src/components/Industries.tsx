...
function DesktopShowcase() {
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      scrub: 0.6,
      onUpdate: (self) => {
        const idx = Math.min(INDUSTRIES.length - 1, Math.floor(self.progress * INDUSTRIES.length));
  ...
  // Jump to a given industry's scroll position using ScrollTrigger's own
  // measured start/end for this trigger, rather than approximating a
  // separate marker element's position (which can drift out of sync with
  // the trigger's actual progress mapping, especially with a `top+=96`
  // start offset, and send the page to a scroll position past real content).
...
export default function Industries() {
  ...
  return (
    <section id="industries" className="lg:py-24 bg-white dark:bg-slate-950 relative overflow-hidden border-b border-slate-200/50 dark:border-slate-800/50 transition-colors duration-300">
