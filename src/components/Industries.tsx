...
function DesktopShowcase() {
  ...
  return (
    <div ref={trackRef} className="hidden lg:block relative" style={{ height: `${INDUSTRIES.length * 55}vh` }}>
          ...
          <div className="col-span-4 space-y-1.5">
            {INDUSTRIES.map((ind, i) => {
              ...
              return (
                <button
                  key={ind.id}
                  onClick={() => scrollToTarget(markerRefs.current[i] as HTMLElement, -160)}
                  data-cursor="hover"
      ...
      {/* Invisible scroll markers used as smooth-scroll anchors for the nav clicks above */}
      {INDUSTRIES.map((_, i) => (
        <div
          key={i}
          ref={(el) => { markerRefs.current[i] = el; }}
          className="absolute left-0 w-px h-px pointer-events-none"
          style={{ top: `${(i / INDUSTRIES.length) * 100}%` }}
        />
      ))}
    </div>
  );
