            ...
            {CORE_VALUES.map((val) => {
              ...
              return (
                <div
                  key={val.title}
                  data-value-card
                  className="card-lift p-6 rounded-2xl bg-white border border-slate-200/60 shadow-sm text-left"
                >
                  <div className="p-2.5 bg-emerald-50 text-brand-primary rounded-xl w-fit mb-4">
                    <IconComp className="w-5 h-5" />
                  </div>
                  <span className="font-display font-bold text-sm text-slate-800 block mb-1.5">{val.title}</span>
